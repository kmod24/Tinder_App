...
class ProfileController: UIViewController {
    ...
    private let bioLabel: UILabel = {
        ...
        label.numberOfLines = 0
        label.font = UIFont.systemFont(ofSize: 20)
        label.text = "I like boba"
        return label
    }()
    
    private lazy var dislikeButton: UIButton = {
        let button = createButton(withImage: UIImage(imageLiteralResourceName: "dismiss_circle"))
        button.addTarget(self, action: #selector(handleDislike), for: .touchUpInside)
        return button
    }()
    
    private lazy var superLikeButton: UIButton = {
        let button = createButton(withImage: UIImage(imageLiteralResourceName: "super_like_circle"))
        button.addTarget(self, action: #selector(handleSuperlike), for: .touchUpInside)
        return button
    }()
    
    private lazy var likeButton: UIButton = {
        let button = createButton(withImage: UIImage(imageLiteralResourceName: "like_circle"))
